...
- **`mcp_server.py`** - Creates the MCP `Server` instance and registers all 30 tool definitions with JSON schemas plus resource handlers. (`list_datastore_clusters` was removed — vCenter-only StoragePod concept) Tool definitions and their handler mappings are both defined here.
